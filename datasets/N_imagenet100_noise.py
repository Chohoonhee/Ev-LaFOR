...
def read_dataset(event_path, noise):
...
class N_Imagenet100_noise(Dataset):
    def __init__(self, prompts, code, preprocess=None, im_size=(224,224), color=False, sub=False, image_dir = '/mnt4/media_from_jm/N_ImageNet/data/ImageNet', event_dir = '/mnt4/media_from_jm//N_ImageNet/data/N_ImageNet',
            split = 'test',  pair= False, test_type = 'zero', clsw =  False, zero_code = None, noise=0.01):
        super().__init__()
        ...
        if self.split == 'train':
            if sub:
                self.data = np.array(self.data)[sorted(np.random.choice(len(self.data), 3000, replace=False))]
    ...
    def __getitem__(self, index):
        ...
        events, event_stack, inverse_event = read_dataset(self.data[index][1], self.noise)
