from models.unet.base_model import BaseModel
import torch.nn as nn
import torch
from models.unet.unet import UNet, UNetRecurrent, UNetDecoder, UNetTask
from os.path import join
from models.unet.submodules import ConvLSTM, ResidualBlock, ConvLayer, UpsampleConvLayer, TransposedConvLayer
...
class E2VIDRecurrent(BaseE2VID):
    ...
    def forward(self, event_tensor, prev_states):
        ...
        """
        img_pred, states, latent = self.unetrecurrent.forward(event_tensor, prev_states)
        return img_pred, states, latent

class E2VIDDecoder(BaseE2VID):
    """
    Recurrent, UNet-like architecture where each encoder is followed by a ConvLSTM or ConvGRU.
    """

    def __init__(self, config):
        super(E2VIDDecoder, self).__init__(config)
        ...
        self.unetrecurrent = UNetDecoder(num_input_channels=self.num_bins,
                                           num_output_channels=1,
                                           skip_type=self.skip_type,
                                           recurrent_block_type=self.recurrent_block_type,
                                           activation='sigmoid',
    ...
    def forward(self, x, blocks, head):
        ...
        :return: reconstructed image, taking values in [0,1].
        """
        img_pred = self.unetrecurrent.forward(x, blocks, head)
        return img_pred

class E2VIDTask(BaseE2VID):
    """
    Recurrent, UNet-like architecture where each encoder is followed by a ConvLSTM or ConvGRU.
    """

    def __init__(self, config):
        super(E2VIDTask, self).__init__(config)
        ...
        self.unetrecurrent = UNetTask(num_input_channels=self.num_bins,
    ...
    def forward(self, input_dict):
        ...
        :return: reconstructed image, taking values in [0,1].
        """
        semseg_pred = self.unetrecurrent.forward(input_dict)
        return semseg_pred
