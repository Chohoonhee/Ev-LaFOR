...
def read_dataset(filename):
    ...
    return torch.tensor(np.array([all_x, all_y, all_ts, all_p]).T, dtype=torch.float32), torch.tensor(event_stack), \
        torch.tensor(np.array([all_x[::-1], all_y[::-1], all_ts.max()-all_ts, all_p[::-1]]).T, dtype=torch.float32)
...
class Caltech101DataEventOursUnpair(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # m = nn.Upsample(size=(224,224), mode='nearest')
        # event_stack = m(event_stack.reshape(1,180,240).unsqueeze(0).repeat(1,3,1,1)).squeeze(0)
        # event_stack2 = m(event_stack.reshape(3,180,240).unsqueeze(0)).squeeze(0)
        # event_stack2 = F.interpolate(event_stack.reshape(3,180,240).unsqueeze(0), size = (224, 224), mode='nearest').squeeze(0)
        return events, label, index, image, event_stack, inv_event
    
    def __len__(self):
        return len(self.data)

class Caltech101DataEventOursUnpairFewshot(Dataset):
    def __init__(self, data_dir = './data/caltech-101/caltech-101/101_ObjectCategories', event_dir = './data/Caltech101',
            split = 'test', train_type = 'zero', pair= False, test_type = 'zero', clsw =  False, preprocess=None, color=None):
        super().__init__()
        self.split = split # This defines the split you are going to use
        ...
        train = open("./data/caltech_train_ours_fewshot20.txt", "r")
        ...
        #for 100 class
        add_label = open("./data/caltech_train_ours_zeroshot20.txt", "r")
        for line in add_label:
            line = line.strip()
            event_list = line.split("&")[0].split(' ')
            label = event_list[0].split('/')[0]
            if label in ["BACKGROUND_Google", "Faces"]:
    ...
    def __getitem__(self, index):
        ...
        events, event_stack = read_dataset(self.data[index][2])
        image_np = np.array(image)
        gray = cv2.cvtColor(image_np, cv2.COLOR_BGR2GRAY)
        ...
        m = nn.Upsample(size=(224,224), mode='nearest')
        # event_stack = m(event_stack.reshape(1,180,240).unsqueeze(0).repeat(1,3,1,1)).squeeze(0)
        event_stack = m(event_stack.reshape(3,180,240).unsqueeze(0)).squeeze(0)
        
        return events, label, index, image, event_stack
