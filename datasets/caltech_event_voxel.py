...
class Caltech101DataEventVoxel(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # image = torch.nn.functional.upsample(image, size=(224, 224), mode='bilinear', align_corners=True)
        # events = np.load(self.data[index][2]).astype(np.float32)
        p, t, x, y, width, height = read_dataset(self.data[index][2])
        ...
        # return events, label
        return event_tensor, label, image
