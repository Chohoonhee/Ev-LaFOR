...
def read_dataset(event_path):
    ...
    # N = event['x'].shape
    # noise = np.random.normal(0, noise * event['t'].max(), N)
    # noise_t = event['t'] + noise
    # inverse_event = np.array([event['x'], event['y'], noise_t - noise_t.min(), event['p'].astype(np.uint8)]).T

    reverse_time = event['t'][::-1]
    inverse_event = np.array([event['x'][::-1], event['y'][::-1], reverse_time.max() - reverse_time , event['p'][::-1].astype(np.uint8)]).T
...
class N_Imagenet100_noise_zero(Dataset):
    def __init__(self, prompts, code, preprocess=None, color=False, sub=False, image_dir = '/media/N_ImageNet/data/ImageNet', event_dir = '/media/N_ImageNet/data/N_ImageNet',
            split = 'test', label_type = 'soft_labels', pair= False, test_type = 'zero', clsw =  False, zero_code = None):
        super().__init__()
        ...
        if self.split == 'train':
            if sub:
                self.data = np.array(self.data)[sorted(np.random.choice(len(self.data), 4000, replace=False))]
